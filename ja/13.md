...
`recsys_django/recsys_django/online/models.py`では`django.db.models.Model`クラスを継承した各モデルを定義しています。ユーザモデル、アイテムモデル、評価値モデルは、それぞれ`User`、`Item`、`Rating`クラスとして定義しています。また、各推薦リストモデルは`Reclist*`クラスとして定義しています。
