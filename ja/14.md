...
body要素は文書の本文に相当するものです。この中にHTMLの本文を記述します。ここでは、ナビゲーションバー、メインコンテンツ、フッターから構成されるようにしています。ここで、メインコンテンツには、`{% block contents %}{% endblock %}`と記述しています。この部分に継承先のテンプレート独自の処理が入ることになります。また、JavaScriptのソースファイルや画像ファイル等への参照は、それぞれ`{# --- js --- #}`、`{# --- files --- #}`以降に記述することにします。

テンプレートの詳細については文献[1]を参照してください．

【ToDo】base.htmlの説明を追記する

ベーステンプレートについては文献[1]を参照してください。
